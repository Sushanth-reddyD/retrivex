...
class ComprehensiveBenchmark:
    ...
    def _create_test_configurations(self) -> List[Dict[str, Any]]:
        ...
        base_config = RetrievalConfig(
            window=2, k=6, token_budget=2000, ordering_strategy=OrderingStrategy.EDGE_BALANCED
        )
        ...
        for budget in [1000, 3000, 4000]:
            config = RetrievalConfig(
                window=2, k=6, token_budget=budget, ordering_strategy=OrderingStrategy.EDGE_BALANCED
            )
        ...
        for strategy in [OrderingStrategy.SCORE_DESC, OrderingStrategy.FRONT_FIRST]:
            config = RetrievalConfig(window=2, k=6, token_budget=2000, ordering_strategy=strategy)
            configs.append({"name": f"retrivex_{strategy.value}", "config": config})

        # k variations
        for k in [4, 8, 10]:
            config = RetrievalConfig(
                window=2, k=k, token_budget=2000, ordering_strategy=OrderingStrategy.EDGE_BALANCED
            )
            configs.append({"name": f"retrivex_k_{k}", "config": config})

        return configs

    def _run_longbench_evaluation(
        ...
        evaluator = LongBenchEvaluator(
            config=eval_config, tasks=["narrativeqa", "qasper", "hotpotqa", "multifieldqa_en"]
        )
    ...
    def _generate_comprehensive_analysis(
        self, benchmark_results: Dict[str, Dict[str, EvaluationMetrics]]
    ) -> Dict[str, Any]:
    ...
    def _analyze_overall_performance(
        ...
        best_method = max(summary["method_rankings"].items(), key=lambda x: x[1]["composite_score"])
    ...
    def _analyze_position_sensitivity(
        self, benchmark_results: Dict[str, Dict[str, EvaluationMetrics]]
    ) -> Dict[str, Any]:
            ...
            for method_name, metrics in results.items():
                ...
                if metrics.front_performance > 0 and metrics.back_performance > 0:
                    ...
                    if front_back_avg > 0:
                        ...
                        analysis["u_shaped_bias_detected"][f"{benchmark_name}_{method_name}"] = (
                            bias_score > 0.1
                        )
                        analysis["middle_performance_drops"][
                            f"{benchmark_name}_{method_name}"
                        ] = bias_score
                ...
                analysis["position_robustness"][f"{benchmark_name}_{method_name}"] = 1.0 / (
                    1.0 + position_variance
                )
        ...
        for benchmark_name, results in benchmark_results.items():
            if edge_balanced_methods and score_desc_methods:
                ...
                analysis["edge_balanced_advantage"][benchmark_name] = (
                    edge_balanced_bias < score_desc_bias
                )
    ...
    def _analyze_efficiency(
        ...
        analysis = {"latency_overhead": {}, "token_efficiency": {}, "scalability": {}}
        ...
        for benchmark_name, results in benchmark_results.items():
            ...
            for method_name, metrics in results.items():
                if "window_" in method_name:
                    ...
                    strategy in method_name
                    for strategy in ["edge_balanced", "score_desc", "front_first"]
                ):
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if best_method and "retrivex" in best_method:
            ...
            recommendations["optimal_configuration"][
                "rationale"
            ] = f"Best overall performance with composite score"
    ...
    def _save_results(
        self, benchmark_results: Dict[str, Dict[str, EvaluationMetrics]], analysis: Dict[str, Any]
    ):
    ...
    def _save_results_csv(
        self, benchmark_results: Dict[str, Dict[str, EvaluationMetrics]], timestamp: str
    ):
    ...
    def _plot_performance_comparison(
        self, benchmark_results: Dict[str, Dict[str, EvaluationMetrics]], fig_dir: Path
    ):
    ...
    def _plot_position_sensitivity(
        self, benchmark_results: Dict[str, Dict[str, EvaluationMetrics]], fig_dir: Path
    ):
