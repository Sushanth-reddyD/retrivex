...
def compute_ndcg_at_k(
    retrieved_ids: List[int], relevance_scores: Dict[int, float], k: int
) -> float:
    """
...
def compute_exact_match(predictions: List[str], references: List[str]) -> float:
    ...
    exact_matches = [
        pred.strip().lower() == ref.strip().lower() for pred, ref in zip(predictions, references)
    ]
...
def compute_batch_f1(predictions: List[str], references: List[str]) -> float:
    ...
    f1_scores = [compute_f1_score(pred, ref) for pred, ref in zip(predictions, references)]

    return np.mean(f1_scores)


def compute_position_bias_metrics(performances: Dict[str, List[float]]) -> Dict[str, float]:
    """
    ...
    front_back_avg = (
        metrics.get("front_performance", 0.0) + metrics.get("back_performance", 0.0)
    ) / 2
...
def compute_token_efficiency_metrics(
    retrieved_texts: List[str], answer_texts: List[str], f1_scores: List[float]
) -> Dict[str, float]:
    """
    ...
    token_efficiencies = [f1 / max(tokens, 1) for f1, tokens in zip(f1_scores, token_counts)]
