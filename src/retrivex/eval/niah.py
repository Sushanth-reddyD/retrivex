...
class NIAHEvaluator(BaseEvaluator):
        ...
        for haystack_length in self.haystack_lengths:
            for needle_depth in self.needle_depths:
                for difficulty in self.needle_difficulties:
                    ...
                    samples_per_config = max(1, min(5, (self.config.max_samples or 100) // 
                                                   (len(self.haystack_lengths) * len(self.needle_depths) * len(self.needle_difficulties))))
    ...
    def _generate_niah_sample(
        ...
        full_context = self._insert_needle_at_depth(
            haystack_text, needle_text, needle_depth
        )
    ...
    def _generate_haystack_content(self, target_length: int, difficulty: str) -> str:
        ...
        while current_length < target_length:
            ...
            available_paragraphs = [p for i, p in enumerate(self.haystack_pool) if i not in used_paragraphs]
